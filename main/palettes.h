...
static const char* const PALETTE_NAMES[PALETTE_COUNT] = {
    "彩虹", "派对", "日落", "岩浆", "热力",
    "梦幻", "海洋", "极光", "森林", "赛博",
    "单色", "随机"
};
...
static inline rgb_t palette_color(uint8_t pid, uint8_t pos) {
    ...
    switch (pid) {
        case PALETTE_RAINBOW: return _hsv2rgb((uint16_t)pos * 360 / 255, 255, 255);
        case PALETTE_PARTY:   return _stops(party, 5, pos);
        case PALETTE_SUNSET:  return _stops(sunset, 4, pos);
        case PALETTE_LAVA:    return _stops(lava, 4, pos);
        case PALETTE_HEAT:    return _stops(heat, 4, pos);
        case PALETTE_SAKURA:  return _stops(sakura, 4, pos);
        case PALETTE_OCEAN:   return _stops(ocean, 4, pos);
        case PALETTE_AURORA:  return _stops(aurora, 4, pos);
        case PALETTE_FOREST:  return _stops(forest, 4, pos);
        case PALETTE_CYBER:   return _stops(cyber, 4, pos);
        case PALETTE_MONO:    return _stops(mono, 2, pos);
        case PALETTE_RANDOM:  return _hsv2rgb(pos, 255, 255);
        default: return _hsv2rgb((uint16_t)pos * 360 / 255, 255, 255);
    }
}
